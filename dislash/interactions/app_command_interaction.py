...
class Resolved:
    def __init__(self, *, data, guild, state):
        self.members: Dict[int, nextcord.Member] = {}
        self.users: Dict[int, nextcord.User] = {}
        self.roles: Dict[int, nextcord.Role] = {}
        self.channels: Dict[int, nextcord.abc.GuildChannel] = {}
        self.messages: Dict[int, nextcord.Message] = {}
        ...
        for ID, data in users.items():
            user_id = int(ID)
            if ID in members:
                self.members[user_id] = nextcord.Member(
                    data={**members[ID], "user": data}, guild=guild, state=state  # type: ignore
                )
            else:
                self.users[user_id] = nextcord.User(state=state, data=data)

        for ID, data in roles.items():
            self.roles[int(ID)] = nextcord.Role(guild=guild, state=state, data=data)

        for ID, data in channels.items():
            data["position"] = 0
            factory, ch_type = nextcord.channel._channel_factory(data["type"])
            if factory:
        ...
        for ID, data in messages.items():
            ...
            self.messages[int(ID)] = nextcord.Message(state=state, channel=channel, data=data)
    ...
    def get(self, any_id: Any) -> Optional[nextcord.abc.Snowflake]:
        any_id = int(any_id)
...
class ContextMenuInteractionData(ApplicationCommandInteractionData):
    ...
    @property
    def member(self):
        if isinstance(self.target, nextcord.Member):
            return self.target

    @property
    def user(self):
        if isinstance(self.target, nextcord.abc.User):
            return self.target

    @property
    def message(self):
        if isinstance(self.target, nextcord.Message):
            return self.target
