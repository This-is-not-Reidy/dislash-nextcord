...
def _partial_emoji_converter(argument: str) -> nextcord.PartialEmoji:
    if len(argument) < 5:
        # Sometimes unicode emojis are actually more than 1 symbol
        return nextcord.PartialEmoji(name=argument)
    ...
    if match is not None:
        ...
        return nextcord.PartialEmoji(name=emoji_name, animated=emoji_animated, id=emoji_id)

    raise nextcord.InvalidArgument(f"Failed to convert {argument} to PartialEmoji")
...
def auto_rows(*buttons: Button, max_in_row: int = 5) -> List[ActionRow]:
    ...
    if not (1 <= max_in_row <= 5):
        raise nextcord.InvalidArgument("max_in_row parameter should be between 1 and 5.")
    return [
...
class SelectOption:
    ...
    def from_dict(cls, data: SelectOptionPayload) -> SelectOption:
        ...
        if emoji is not None:
            emoji = nextcord.PartialEmoji.from_dict(emoji)
        return cls(
            label=data["label"],
            value=data["value"],
...
class Button(Component):
    ...
    label : :class:`str`
        Button text
    emoji : :class:`str` | :class:`nextcord.PartialEmoji`
        Button emoji
    custom_id : :class:`str`
    ...
    style: ButtonStyle
    label: Optional[str]
    emoji: Optional[nextcord.PartialEmoji]
    url: Optional[str]
    ...
    def __init__(
        ...
        *,
        ...
        emoji: Optional[Union[nextcord.PartialEmoji, str]] = None,
        custom_id: Optional[str] = None,
        ...
        if custom_id is None:
            if url is None:
                ...
                raise nextcord.InvalidArgument("if you specify url, the style must be ButtonStyle.link")
        elif url is not None:
            raise nextcord.InvalidArgument("you can't specify both url and custom_id")
        elif style == ButtonStyle.link:
            raise nextcord.InvalidArgument("style 'link' expects url to be specified")
...
class ActionRow(Component):
    ...
    def __init__(self, *components: Component):
        ...
        if len(components) > self._limit:
            raise nextcord.InvalidArgument(f"components must be a list of up to {self._limit} elements")
        if not all(isinstance(comp, Component) for comp in components):
            raise nextcord.InvalidArgument("components must be a list of Component")
