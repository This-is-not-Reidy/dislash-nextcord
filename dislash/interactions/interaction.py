...
class BaseInteraction:
    ...
    def __init__(self, client: Client, data: dict):
        ...
        self.guild: Optional[nextcord.Guild]
        self.channel_id: Optional[int]
        self.channel: Optional[Union[nextcord.abc.GuildChannel, nextcord.abc.Thread, nextcord.abc.PrivateChannel]]
        ...
        if "guild_id" in data:
            ...
            self.author = nextcord.Member(data=data["member"], guild=self.guild, state=state)
        else:
            self.guild_id = None
            self.guild = None
            self.author = nextcord.User(state=state, data=data["user"])
    ...
    async def reply(
        ...
        *,
        ...
        embed : :class:`nextcord.Embed`
            message embed
        embeds : :class:`List[nextcord.Embed]`
        ...
        view : :class:`nextcord.ui.View`
            only usable with nextcord.py 2.0. Read more about ``View`` in
            nextcord.py 2.0 official documentation
        file : :class:`nextcord.File`
            if it's the first interaction reply, the file will be ignored due to API limitations.
            Everything else is the same as in :class:`nextcord.TextChannel.send()` method.
        ...
        Returns
        -------
        message : :class:`nextcord.Message` | ``None``
        ...
        if self._sent or self.expired or type == 3:
            ...
            if self.bot.slash._uses_nextcord_2:
    ...
    async def create_response(
        ...
        *,
        ...
        if embed is not None and embeds is not None:
            raise nextcord.InvalidArgument("Can't pass both embed and embeds")

        if embed is not None:
            if not isinstance(embed, nextcord.Embed):
                raise nextcord.InvalidArgument("embed parameter must be nextcord.Embed")
            data["embeds"] = [embed.to_dict()]

        elif embeds is not None:
            if len(embeds) > 10:
                raise nextcord.InvalidArgument("embds parameter must be a list of up to 10 elements")
            elif not all(isinstance(embed, nextcord.Embed) for embed in embeds):
                raise nextcord.InvalidArgument("embeds parameter must be a list of nextcord.Embed")
            data["embeds"] = [embed.to_dict() for embed in embeds]

        if view is not None:
            if not hasattr(view, "__nextcord_ui_view__"):
                raise nextcord.InvalidArgument(f"view parameter must be View not {view.__class__!r}")
        ...
        if components is not None:
            if len(components) > 5:
                raise nextcord.InvalidArgument("components must be a list of up to 5 action rows")
    ...
    async def edit(
        ...
        *,
        ...
        if components is not None:
            if len(components) > 5:
                raise nextcord.InvalidArgument("components must be a list of up to 5 elements")
            if not all(isinstance(comp, ActionRow) for comp in components):
                raise nextcord.InvalidArgument("components must be a list of ActionRow")
    ...
    async def followup(
        ...
        *,
        ...
        if embed is not None and embeds is not None:
            raise nextcord.InvalidArgument("cannot pass both embed and embeds parameter to followup()")
        ...
        if file is not None and files is not None:
            raise nextcord.InvalidArgument("cannot pass both file and files parameter to followup()")
