...
class OptionParam:
    ...
    TYPES: Dict[Hashable, int] = {
        ...
        nextcord.Role: 8,
        Union[nextcord.Member, nextcord.Role]: 9,
        nextcord.abc.Snowflake: 9,
        float: 10,
    }
...
class SlashCommand(ApplicationCommand):
    ...
    name : :class:`str`
        The command name
    description : :class:`str`
        The command description (it'll be displayed by nextcord)
    options : List[Option]
...
class ApplicationCommandPermissions:
    ...
    @classmethod
    def from_pairs(cls, permissions: Dict[Union[nextcord.Role, nextcord.User], bool]) -> ApplicationCommandPermissions:
        """
        Creates :class:`ApplicationCommandPermissions` using
        instances of :class:`nextcord.Role` and :class:`nextcord.User`
...
class RawCommandPermission:
    ...
    @classmethod
    def from_pair(cls, target: Union[nextcord.Role, nextcord.User], permission: bool) -> RawCommandPermission:
        if not isinstance(target, (nextcord.Role, nextcord.User)):
            raise nextcord.InvalidArgument("target should be Role or User")
        if not isinstance(permission, bool):
            raise nextcord.InvalidArgument("permission should be bool")
        return RawCommandPermission(
            id=target.id, type=1 if isinstance(target, nextcord.Role) else 2, permission=permission
        )
