...
class InteractionClient:
    ...
    ----------
    client : :class:`commands.Bot` | :class:`commands.AutoShardedBot`
        an instance of any class inherited from :class:`nextcord.Client`
    ...
    def __init__(
        ...
        *,
        ...
        self._uses_nextcord_2 = hasattr(client, "add_view")
        ...
        # Modify old nextcord.py methods
        self._modify_nextcord()
    ...
    def _modify_nextcord(self) -> None:
        # Modify cog loader
        ...
        if self._modify_send:
            if self._uses_nextcord_2:
                ...
                nextcord.state.create_message = create_message_with_components
            nextcord.abc.Messageable.send = send_with_components
            nextcord.Message.edit = edit_with_components

        commands.Context.wait_for_button_click = ctx_wait_for_button_click
        nextcord.Message.create_click_listener = create_click_listener
        nextcord.Message.wait_for_button_click = message_wait_for_button_click
        nextcord.Message.wait_for_dropdown = message_wait_for_dropdown
        nextcord.Guild.get_commands = get_commands
        nextcord.Guild.get_command = get_command
        nextcord.Guild.get_command_named = get_command_named
        nextcord.Guild.fetch_commands = fetch_commands
        nextcord.Guild.fetch_command = fetch_command
        nextcord.Guild.edit_command = edit_command
        nextcord.Guild.edit_command_permissions = edit_command_permissions
        nextcord.Guild.batch_edit_command_permissions = batch_edit_command_permissions
        nextcord.Guild.delete_command = delete_command
        nextcord.Guild.delete_commands = delete_commands
    ...
    def slash_command(self, *args, **kwargs) -> Callable[[Callable[..., Awaitable]], CommandParent]:
        ...
        description : :class:`str`
            the description of the slash command. It will be visible in nextcord.
        options : :class:`List[Option]`
            the list of slash command options. The options will be visible in nextcord.
    ...
    async def register_global_command(self, app_command: ApplicationCommand):
        ...
        if not isinstance(app_command, ApplicationCommand):
            raise nextcord.InvalidArgument("Expected an ApplicationCommand instance")
    ...
    async def register_guild_command(self, guild_id: int, app_command: ApplicationCommand):
        ...
        """
        if not isinstance(app_command, ApplicationCommand):
            raise nextcord.InvalidArgument("Expected a ApplicationCommand instance")
        r = await self.client.http.request(
            Route(
                "POST",
    ...
    async def overwrite_global_commands(self, app_commands: list):
        ...
        if not all(isinstance(sc, ApplicationCommand) for sc in app_commands):
            raise nextcord.InvalidArgument("app_commands must contain only ApplicationCommand instances")
    ...
    async def edit_global_command(self, command_id: int, app_command: ApplicationCommand, **kwargs):
        ...
        """
        if not isinstance(app_command, ApplicationCommand):
            raise nextcord.InvalidArgument("parameter app_command must be ApplicationCommand")
    ...
    async def _maybe_unregister_commands(self, guild_id: Optional[int]):
        ...
        it still exists in nextcord and creates interactions.
        """
        if guild_id is None:
            return
    ...
    async def _on_ready(self):
        if isinstance(self.client, nextcord.AutoShardedClient):
            self.is_ready = True
            await self._activate_event("ready")
    ...
    async def _process_interaction(self, payload: Dict[str, Any]):
        event_name = "dislash_interaction" if self._uses_nextcord_2 else "interaction"
