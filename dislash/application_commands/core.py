...
class InvokableApplicationCommand:
    def __init__(self, func: Callable[..., Awaitable], *, name: str = None, **kwargs):
        ...
        if cooldown is None:
            try:
                # Assuming that it's nextcord.py 1.7.0+
    ...
    def _isinstance(obj: Any, typ: type) -> bool:
        if not isinstance(typ, type) or isinstance(typ, EnumMeta):  # special annotation
            return True
        if issubclass(typ, nextcord.User):
            return isinstance(obj, (nextcord.User, nextcord.Member, nextcord.ClientUser))
        else:
...
def has_role(item: Union[int, str]):
    ...
    def predicate(ctx):
        if not isinstance(ctx.channel, nextcord.abc.GuildChannel):
            raise NoPrivateMessage()

        if isinstance(item, int):
            role = nextcord.utils.get(ctx.author.roles, id=item)
        else:
            role = nextcord.utils.get(ctx.author.roles, name=item)
        if role is None:
...
def bot_has_role(item: Union[int, str]):
    ...
    def predicate(ctx):
        ch = ctx.channel
        if not isinstance(ch, nextcord.abc.GuildChannel):
            raise NoPrivateMessage()
...
def is_nsfw():
    ...
    def pred(ctx):
        ch = ctx.channel
        if ctx.guild is None or (isinstance(ch, nextcord.TextChannel) and ch.is_nsfw()):
            return True
...
def cooldown(rate: int, per: float, type: BucketType = BucketType.default):
    """
    A decorator that adds a cooldown to a slash-command. Similar to **nextcord.py** cooldown decorator.
